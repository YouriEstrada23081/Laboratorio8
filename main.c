...
volatile int carrera=0;

volatile int carrera1=0;
...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  uint8_t Test2[]="Mande S para iniciar\r\n";
  ...
  while (1)
  {
	  ...
	  switch (estado){
	  case 0:
		  if(carrera==1 && carrera1==0){
		  uint8_t Test3[]="Mande S para iniciar \r\n";
		  			  HAL_UART_Transmit(&huart2,Test3,sizeof(Test3),1000);
		  }
		  carrera1=carrera;
		  break;
	  case 1:
	    ...
	    switch (contador1) {
		  ...
		  if(contador==5){
			  uint8_t Test[]="El ganador es el jugador 1  \r\n";
			  HAL_UART_Transmit(&huart2,Test,sizeof(Test),1000);
		      estado=0;
		      carrera=1;
		  }
		  if(contador1==5){
			  uint8_t Test1[]="El ganador es el jugador 2  \r\n";
			  HAL_UART_Transmit(&huart2,Test1,sizeof(Test1),1000);
		      estado=0;
		      carrera=1;
		  }
